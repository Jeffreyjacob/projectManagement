...
const ReusablePriorityPage = ({priority}: Props) => {
    ...
    const id  = 1;
    const {data:task,isLoading,isError:isTasksError} = useGetTaskbyUserQuery(id || 0,{
        skip:id === null
    });

    const isDarkMode = useAppSelector((state)=> state.global.isDarkMode)
    const filteredTask = task?.filter((task:Task)=> task.priority === priority)
    
    if(isTasksError ||!task) return <div>Error fetch task</div>;
  return (
    <div className='m-5 p-4'>
      <ModelNewTask isOpen={isModalNewTaskOpen} 
